...
export const AlbumCard = ({ data, index }) => {
  ...
  const [{ allAlbum }, dispatch] = useStateValue();
  const handleDelete = (id) => {
    deleteAlbum(id).then((data) => {
      dispatch({ type: actionType.SET_ALL_ALBUMNS, allAlbum: data.data });
    });
  };
  return (
    <motion.div
      ...
      {isDelete && (
        ...
        >
          ...
          <div className="flex items-center w-full justify-center gap-3">
            <div className="bg-red-300 px-3 rounded-md" onClick={() => handleDelete(data._id)}>
              <p className="text-headingColor text-sm">Yes</p>
            </div>

            <div
              className="bg-green-300 px-3 rounded-md" onClick={() => setIsDelete(false)}>
